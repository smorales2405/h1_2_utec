...
def _config_path() -> Path:
    ...
    Aquí no hay `ament_index` ni `share/`: el fichero vive junto al código, que
    es como se despliega en el robot. `H12_GAINS` lo fuerza.
    """
    import os
    ...
    for c in (aqui / "gains.yaml", aqui.parent / "gains.yaml"):
        if c.exists():
            return c
    return aqui.parent / "gains.yaml"
